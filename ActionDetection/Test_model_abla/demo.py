...
with open('/Users/mac/Documents/2CS_IASD/S2/PROJET_2CS/Action_recognition_3dcnn/Project-2CS/ActionDetection/Test_model_abla/configs.json') as data_file:
...
loaded_dict = torch.load(curr_folder + '/demo.ckp', map_location=torch.device('cpu'))  # Load on CPU
...
while True:
    ...
    if len(imgs) == 16:
        ...
        out = (torch.nn.Softmax()(output).data).numpy()[0]
        if len(hist) > 300:
        ...
        mean_hist.append(curr_mean.numpy())
    ...
    resized_frame = cv2.resize(frame, (1200, 480))
    bg[:480, :1200] = resized_frame
