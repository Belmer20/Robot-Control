...
class Topic(object):
    ...
    def register_node(self, node_name=None):
        if node_name is None and self.allow_anonymous:
            return False
        if node_name not in self.publishers:
            publisher = rospy.Publisher(self.name, self.message_type, queue_size=self.queue_size)
            self.publishers_names.append(node_name)
            self.publishers[node_name] = publisher
            return publisher
        return False

    def register_publisher(self, node_name=None):
        if node_name is None and self.allow_anonymous:
            return False
        if node_name not in self.publishers:
            publisher = rospy.Publisher(self.name+"_"+str(node_name[-1]), self.message_type, queue_size=self.queue_size)
            self.publishers_names.append(node_name)
            self.publishers[node_name] = publisher
            return publisher
        return False
